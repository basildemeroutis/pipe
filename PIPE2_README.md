...
Before reading anything else, internalise these two principles. Every architectural decision in PIPE2 flows from them.
...
| Excel model work (cashflow, appraisals) | `excel-auditor` |
| Meeting transcript analysis | `meeting-insights-analyzer` |
| Marketing copy or persuasion architecture | `marketing-psychology` |
| Email drafting and inbox management | **Superhuman** *(no skill file yet — use natively)* |
| Governance documents, investor memos | **governance-drafting** *(skill exists at `/mnt/skills/user/governance-drafting/` — verify path before use)* |

**Not relevant to PIPE2:** `fore-brand-guidelines` (FORE brand identity, not a coding tool).
...
5. **Session continuity** — Users resume work across conversations, devices, days. Database persists 30 days. A session is a property, not a conversation.
6. **Middleware responses are instructions** — Vals tell GPT what to do next: `next_actions`, `post_store_actions`, `transition_message`. The response *is* the instruction set. GPT reads these and complies — it doesn't decide what to do next.
...
|-----|----------------|-------------|
| `pipe-epc-imd` | EPC certificate (~25 fields: ratings, floor area, emissions, construction) + DEC (operational rating for public buildings) + IMD deprivation indices (~13 fields) | `EPC`, `IMD` |
...
| `pipe-breeam` | BREEAM sustainability certification lookup. Most buildings have no cert — "Not certified" is normal. **BLOCKED: awaiting API credentials from breeam@bregroup.com** | `BREEAM` |
...
`pipe-crrem /crrem1` — base case (do-nothing scenario). Calculates stranding year against CRREM V2.07 UK 1.5°C pathway. Uses EPC carbon data, grid decarbonisation curve, UK Treasury £241/tCO₂ for VAR calculation.

`pipe-crrem /crrem2` — retrofit scenario. Calls `pipe-energy` internally (never directly by GPT). Physics-based energy model using 24 CIBSE/SBEM/NCM/TM46 lookup tables. Returns 27-year staircase (2024–2050) with intervention impact per year. User and GPT collaborate on intervention packages; user says "lock in that retrofit strategy" to commit to database.
...
Current production versions. **Always verify against deployed Val health check before editing.**

| Val | Version | URL | GPT-facing? | Purpose |
|-----|---------|-----|-------------|---------|
| pipe-start | 2.0.3 | pipe-start.val.run | ✓ `init`, `address` | Session init + address lookup (Ideal Postcodes / AddressBase Core) |
| pipe-epc-imd | 2.0.0 | pipe-epc-imd.val.run | ✓ `buildingData` | **Merged Val:** EPC Register + DEC (operational ratings) + IMD deprivation |
| pipe-property | 2.1.0 | pipe-property.val.run | ✓ `planning`, `heritage`, `hmlr`, `transport`, `breeam` | **Merged Val:** PLD + PlanIt planning / Historic England / Land Registry + proxy routes for Transport and BREEAM |
| pipe-planning | 2.0.2 | pipe-planning.val.run | — internal | Standalone planning engine called by pipe-property (PLD + PlanIt) |
| pipe-companies | 1.0.0 | pipe-companieshouse.val.run | — auto-fired | Companies House — auto-fired from pipe-property's HMLR handler when corporate owner detected. Not a GPT action. |
| pipe-transport | 1.0.0 | pipe-transport.val.run | — downstream | TfL transport accessibility. London only. Called by pipe-property `/transport` proxy. |
| pipe-breeam | 1.0.0 | pipe-breeam.val.run | — downstream | BREEAM certification lookup. Called by pipe-property `/breeam` proxy. **BLOCKED — awaiting credentials** |
| pipe-costs | 1.0.5 | pipe-costs.val.run | ✓ `costsPreview`, `costsConfirm`, `costsEvidence` | Cost benchmarking (in development) |
| pipe-crrem | 2.0.7 | pipe-crrem.val.run | ✓ `crrem1`, `crrem2` | CRREM V2.07 base case + retrofit. Calls pipe-energy internally. |
| pipe-energy | 1.1.1 | pipe-energy.val.run | — internal | Physics-based energy model. 24 CSV lookup tables. Called only by pipe-crrem. |
| pipe-store | 2.2.1 | pipe-store.val.run | ✓ `store` | Field validation + storage gateway. The only write path to SQLite. |
| pipe-session | 1.7.5 | pipe-session.val.run | ✓ `pull`, `status`, `questions`, `progress`, `next` | Session state, phase tracking, question serving |
| pipe-dropzone | 1.2.5 | pipe-dropzone.val.run | — user-facing | HTML upload UI. Bypasses GPT entirely. |
| pipe-dropzone-control | 1.1.2 | pipe-dropzone-control.val.run | ✓ `dropInit`, `dropCheck` | Upload token management + ingest status |
| pipe-dropzone-status | 1.1.2 | pipe-dropzone-status.val.run | — internal | Drop Zone upload progress |
| pipe-report | 1.5.2 | pipe-report.val.run | ✓ `generateReport` | Report generation orchestration |
| pipe-field-guide | 2.1.4 | esm.town/v/basild/PIPE_field_guide | — import | Field definitions + validation rules |
| pipe-admin | 1.1.1 | pipe-admin.val.run | — standalone UI | Admin dashboard. No auth. Internal use only. |
| pipe-imd-refresh | 1.0.2 | pipe-imd-refresh.val.run | — batch | IMD CSV batch import |
| pipe-imd-refresh-cron | 1.0.1 | (cron) | — cron | Scheduled IMD refresh |
| pipe-userQues | 1.0.1 | pipe-userQues.val.run | ✓ `userQues` | User question loader |
| fore-services | 3.4.8 | fore-services.val.run | ✓ `getTime`, `trigger`, `getLoader`, `searchDocs` | INIT support + knowledge search |

---

## Context Binders (GPT Knowledge Files)

PIPE2 GPT has 240+ specialist documents loaded into active memory — not publicly available and not part of standard LLM training. This is what enables contextual reasoning beyond API data: "Does this comply with Part M?", "What BREEAM credit applies here?", "How would FORE frame this investment thesis?"

| Domain | Count | Examples |
|--------|-------|---------|
| UK Regulation | 25+ | Part L, Part M, MEES, EPC methodology, Building Safety Act |
| Sustainability Standards | 35+ | BREEAM V7, NABERS UK, CRREM, GRESB, UKGBC frameworks |
| Retrofit Guidance | 20+ | LETI Climate Emergency Guide, BBP Retrofit Pathway, CIBSE TM54/TM65 |
| Circular Economy | 15+ | Ellen MacArthur Foundation, WorldGBC Playbook, GLA Circular Design |
| FORE/MORE Materials | 40+ | Investment letters, case studies, fund documents |
| Finance & ESG | 30+ | TCFD, SFDR, CSRD/ESRS, green premium research |
| Innovation & PropTech | 20+ | Christensen disruption theory, Rogers diffusion |
| Urbanism & Psychology | 25+ | Loneliness research, spatial justice, behavioural design |
| Climate Science | 15+ | IPCC AR6, planetary boundaries, emissions gap reports |

These are the source material for GPT-synthesised fields (Phase 4 and 7). When `epc_commentary` or `planning_history` is proposed, it draws on this corpus. When GPT proposes an intervention sequence, it's reasoning against CIBSE Guide F benchmarks and CRREM methodology — not guessing.

**For developers:** The Context Binders are managed in the ChatGPT custom GPT knowledge file settings, not in Val.town. They are not fetchable by Vals and are invisible to middleware.
...
**Not GPT actions** (internal/auto-fired): pipe-companieshouse (auto-fired from HMLR handler), pipe-transport (proxied via pipe-property), pipe-breeam (proxied via pipe-property), pipe-planning (proxied via pipe-property), pipe-energy (called by pipe-crrem), pipe-userQues (questions loaded via Drop Zone, not GPT action).
...
## PythonAnywhere Flask App (v1.3.0)
...
**Templating:** docxtpl (`{{ field_name }}` Jinja2 placeholders). Images via `InlineImage` at 60mm square. Number formatting applied: `£`, `sq ft`, `tCO₂`.
...
| EPC Register | pipe-epc-imd | `epc_auth_token` | Returns empty; does not block pipeline |
...
| Historic England (NHLE) | pipe-property | None (public) | "Not listed" stored explicitly; CA fallback via LPA |
| Land Registry (HMLR) | pipe-property | None (public, Turso) | PRISM self-healing: postcode in wrong param auto-corrected |
...
| `epc_auth_token` | pipe-epc-imd | EPC Register Basic Auth token |
| `TURSO_OWNERSHIP_URL` | pipe-property | Turso DB URL (HMLR data, without libsql://) |
| `TURSO_TOKEN` | pipe-property | Turso auth token |
...
```
USER, GPT, OS_PLACES, EPC, DEC, IMD, COMPANIES_HOUSE, LAND_REGISTRY, PLANNING,
GOOGLE_PLACES, CALCULATED, BROCHURE, CONTEXT_BINDER, DOCUMENT, USER_TEMPLATE,
DROPZONE, PLD, PLANIT, NHLE, HE_CA, HMLR, CCOD, OCOD, POSTCODES_IO,
IDEAL_POSTCODES, BENCHMARK, FALLBACK, TFL, BREEAM
```
...
Fast infrastructure check. No GPT, no Chrome, no UI. Tests 13 Vals, 26 routes.
...
| OpenAPI schemas | `_Claude_resources/openapi-pipe-*.json` |
| Flask app | PythonAnywhere `/home/BasilDemeroutis/PIPE2/flask_app.py` |
| Sample report PDF | `www.forepartnership.com/pipe/PIPE_report_example.pdf` |
| Smoke test | `_Claude_resources/pipe2-test-harness/smoke-test.py` |
| Test harness skill | `_Claude_resources/pipe2-test-harness/SKILL.md` |
...
*PIPE2_README.md | v1.1.0 | 7 March 2026*
